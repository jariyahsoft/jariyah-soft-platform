...
**Document version:** 4.6-Opus (Final Consolidated)  
**Status:** Development-ready specification  
**Consolidated from:** chatgpt5.5.md, gemini3.1pro.md, gemini3.5flash.md  
...
# 22. Firebase Storage Rules

## 22.1 Directory Structure

```text
/users/{uid}/avatar.{ext}
/software/{softwareId}/logo.{ext}
/software/{softwareId}/screenshots/{filename}.{ext}
/articles/{articleId}/cover.{ext}
/articles/{articleId}/images/{filename}.{ext}
...
service firebase.storage {
  match /b/{bucket}/o {
    ...
    // Article images
    match /articles/{articleId}/cover.{ext} {
      allow read: if true;
      allow write: if signedIn()
                   && request.auth.token.role in ['developer', 'admin']
                   && isValidImage();
    ...
    match /articles/{articleId}/images/{filename} {
      allow read: if true;
      allow write: if signedIn()
                   && request.auth.token.role in ['developer', 'admin']
                   && isValidImage();
    ...
    // Certificates (server-generated only)
...
* ตรวจ MIME signature จริง (magic bytes) ใน Cloud Functions ก่อน finalize ไม่พึ่ง extension เพียงอย่างเดียว
* ไฟล์ที่ตรวจ malware scan ไม่ผ่านจะถูก quarantine และลบภายใน 24 ชั่วโมง
* ตั้ง CORS ให้อนุญาตเฉพาะ origin ของ production/staging domain
* เปิด App Check enforcement สำหรับ Storage เหมือนกับ Firestore
* Lifecycle rule: ลบ orphaned files (ไม่มี document อ้างอิง) ที่อายุเกิน 30 วัน ผ่าน scheduled function

---

# 23. Caching และ CDN Strategy

## 23.1 Next.js Rendering Strategy
...
* Static assets (images, fonts, CSS/JS bundles): immutable hash filename + `Cache-Control: public, max-age=31536000, immutable`

## 23.4 CDN

* Firebase Hosting หรือ Vercel Edge Network ทำหน้าที่ CDN สำหรับ static assets และ ISR pages
* Storage images ให้ serve ผ่าน Firebase Storage CDN URL (`firebasestorage.googleapis.com`) หรือ custom domain ที่ตั้ง Cloud CDN
* ตั้ง `Vary: Accept-Encoding` สำหรับ text-based assets

---

# 24. Monitoring, Observability และ Alerting

## 24.1 Structured Logging

* ทุก Cloud Function และ API endpoint log เป็น JSON format ผ่าน Cloud Logging
* Log fields บังคับ: `severity`, `requestId`, `userId` (hashed ถ้า needed), `action`, `duration_ms`, `status_code`
...
## 24.2 Metrics และ Dashboards

| Metric | Source | Dashboard |
|---|---|---|
| API request rate / latency / error rate | Cloud Functions metrics | Cloud Monitoring |
| Firestore read/write/delete operations | Firebase Console | Cloud Monitoring |
| Storage upload size / count | Firebase Storage metrics | Cloud Monitoring |
| Search query latency / zero-result rate | Typesense Cloud dashboard + custom log | Custom dashboard |
| Active users (DAU/MAU) | Google Analytics | GA4 / Looker Studio |
| Moderation queue backlog | Firestore aggregate query | Admin Dashboard |
| Notification delivery success/failure | Custom `notifications` collection aggregation | Admin Dashboard |
| CI/CD deployment frequency / lead time | GitHub Actions metrics | Custom dashboard |

## 24.3 Alerting Rules

| Condition | Severity | Channel | Action |
|---|---|---|---|
| API error rate > 2% ใน 5 นาที | Critical | PagerDuty / Line | On-call investigate |
| API p95 latency > 1,000 ms ใน 10 นาที | Warning | Email / Slack | Investigate |
| Firestore daily read > 80% of budget | Warning | Email | Review query patterns |
| Search sync queue lag > 5 นาที | Warning | Slack | Check Cloud Functions |
| Search sync dead-letter > 0 | Critical | PagerDuty | Manual reconciliation |
| Notification failure rate > 5% | Warning | Email | Check email provider |
| Zero successful deployments in 7 days | Info | Email | Review CI/CD |
| Moderation backlog > SLA threshold | Warning | Slack / Email | Assign moderators |

## 24.4 Health Checks

* `/api/health` endpoint (public, no auth) ที่ตรวจ Firestore connectivity, Typesense connectivity และ return status
* Scheduled function ทุก 5 นาทีตรวจ external dependencies (GitHub API, email provider) และบันทึกใน `system_metrics`
* Uptime check จาก Cloud Monitoring ที่ `/api/health` ทุก 1 นาที

---

# 25. Internationalization (i18n) Implementation

## 25.1 Architecture

* ใช้ `next-intl` หรือ `next-i18next` สำหรับ i18n routing (`/th/...`, `/en/...`)
* Default locale: `th`; Supported locales: `th`, `en`
* ภาษาถูกเก็บใน `users.locale` และ browser `Accept-Language` สำหรับ Guest
* URL structure: `/{locale}/software/{slug}`, `/{locale}/articles/{slug}`

## 25.2 Translation Files

```text
/locales
  /th
    common.json     # UI labels, buttons, navigation
    software.json   # Software Hub specific
    knowledge.json  # Knowledge Hub specific
...
## 25.3 Content Localization

* **UI strings**: เก็บใน JSON translation files, ห้าม hard-code ข้อความ UI ในโค้ด
* **User-generated content** (articles, software descriptions): เก็บใน single language ต่อ document โดยมี `language` field
* **Master data** (categories, badges, learning paths): ใช้ `localizedMap` format เช่น `{ "th": "ชื่อ", "en": "Name" }`
* **Date/Time display**: ใช้ `Intl.DateTimeFormat` แสดงตามปฏิทิน (Buddhist Era สำหรับ `th`, Gregorian สำหรับ `en`); API ใช้ ISO 8601 UTC เสมอ
* **Number formatting**: ใช้ `Intl.NumberFormat` สำหรับ download count, rating

## 25.4 SEO per Locale

* `<html lang="th">` หรือ `<html lang="en">` ตาม locale
* `<link rel="alternate" hreflang="th" href="..." />` และ `hreflang="en"` ในทุกหน้า
* Sitemap แยกต่อ locale หรือใช้ `xhtml:link` alternatives
* Open Graph `og:locale` ตั้งตาม locale ของหน้า

---

# 26. Appeal Process (Detailed)

## 26.1 Eligibility

* Developer สามารถยื่นอุทธรณ์ได้ **หนึ่งครั้ง** ต่อ rejection decision ภายใน **14 วัน** นับจากวันที่ถูก reject
* ห้ามยื่นอุทธรณ์ซ้ำสำหรับ decision เดิม
* เนื้อหาที่ถูก takedown ด้วยเหตุผล Malware, Ransomware หรือ Illegal Content **ไม่สามารถอุทธรณ์ได้**
...
stateDiagram-v2
    [*] --> Rejected
    Rejected --> AppealSubmitted: Developer submits appeal
    AppealSubmitted --> AppealUnderReview: Moderator claims
    AppealUnderReview --> Overturned: Appeal accepted
    AppealUnderReview --> Upheld: Appeal denied
    Overturned --> Draft: Content returned for edit
    Upheld --> [*]: Final decision
    Overturned --> [*]
```
...
| Field | Type | Req. | รายละเอียด |
|---|---|---:|---|
| resourceType | enum | yes | `software`, `article` |
| resourceId | string | yes | ID ของ resource ที่ถูก reject |
| originalDecisionId | string | yes | อ้างถึง audit_log entry ของ rejection |
| developerId | string | yes | UID ของ developer ที่ยื่น |
| reason | string | yes | เหตุผลของ developer (สูงสุด 2,000 ตัวอักษร) |
| attachments | string[] | no | Storage paths ของเอกสารประกอบ |
| status | enum | yes | `submitted`, `under_review`, `overturned`, `upheld` |
| reviewerId | string | no | Moderator/Admin ที่รับตรวจ (ต้องไม่ใช่คนเดิมที่ reject) |
| reviewNote | string | no | หมายเหตุจาก reviewer |
| createdAt | timestamp | yes | วันที่ยื่น |
| resolvedAt | timestamp | no | วันที่ตัดสิน |

Indexes: `(status, createdAt)`, `(developerId, createdAt desc)`, `(resourceType, resourceId)`

## 26.4 Business Rules

* Moderator ที่ reject ครั้งแรกห้ามเป็นคนตรวจ appeal (ป้องกัน conflict of interest)
* Appeal ที่ไม่ได้รับการตรวจภายใน 14 วัน ให้ escalate ไป Administrator อัตโนมัติ
* ทุก decision ต้องบันทึก Audit Log และส่ง Notification แจ้ง Developer
* หาก overturn ระบบจะเปลี่ยนสถานะ resource กลับไป `draft` ให้ developer แก้ไขและ submit ใหม่
...
| **Custom Claims** | ข้อมูลเพิ่มเติมใน Firebase Auth token (เช่น role) |
...
|---|---|---|---|
| 1.0 (chatgpt.md) | 2026-06 | ChatGPT | High-level SRS: 36 sections covering all functional modules, basic Firestore collections, roadmap |
| 5.5 (chatgpt5.5.md) | 2026-06 | ChatGPT | Development-ready spec: detailed Firestore schema with indexes, full API spec, Security Rules code, search implementation, comprehensive user stories, notification templates, error handling matrix, data migration strategy, CI/CD pipeline |
| 3.1-Pro (gemini3.1pro.md) | 2026-06 | Gemini 3.1 Pro | Extended from 1.0: added API endpoints, Firestore field definitions, security rules description, Member user stories, search technology details, wireframes, notification templates, data seeding |
| 3.5-Flash (gemini3.5flash.md) | 2026-06 | Gemini 3.5 Flash | Based on 5.5: added US-014 Member profile/privacy management |
| **4.6-Opus** | **2026-06** | **Claude Opus 4.6** | **Consolidated final version: merged all sources + added 6 new sections: Firebase Storage Rules (code), Caching & CDN Strategy, Monitoring & Observability (metrics, alerting, health checks), i18n Implementation (architecture, translation files, SEO per locale), Appeal Process (detailed workflow, data model, business rules), Glossary** |
